    ...
    def test_search(self):
        ...
        with self.subTest("Storing search results to be compared later"):
            for index in self.indexes_to_test_on:
                ...
                for query, search_method, result_key in zip(self.queries, self.search_methods, self.result_keys):
                    self.logger.info(f"for loop variables are query: {query}, search_method: {search_method}, result_key: {result_key}")
                    all_docs = self.client.index(index_name).get_documents()
                    self.logger.info(f"Printing all documents after transferring state for index {index_name}, docs: {all_docs}")
                    if index.get("type") is not None and index.get("type") == 'structured':
                        if search_method == 'HYBRID':
                            result = self.client.index(index_name).search(q=query, search_method=search_method, hybrid_parameters=self.hybrid_search_params)
                        else:
                        ...
                        result = self.client.index(index_name).search(q=query, search_method=search_method)
                    self.logger.info(f"Got result: {result} for index: {index_name}, query: {query}, search_method: {search_method}")
                    self.logger.info(f"The equivalent stored result for this is {stored_results[index_name][result_key]}")
